...
- `relativePath` (string) — путь назначения относительно `projectRoot`.
  ...
  `relativePath` (замена `<agloomDir>/agents/` на `<adapter.targetDir>/`)
  в операции «Транспиляция», шаг 3. После ремаппинга `relativePath`
  содержит целевой путь (например, `".claude/agents/code-reviewer.md"`).
...
1. Определить путь к каталогу agents как
   `<projectRoot>/<agloomDir>/agents/`.
2. Проверить наличие каталога agents.
3. Получить список прямых дочерних файлов каталога agents.
4. Отфильтровать файлы, оставив только файлы с расширением `.md`.
5. Прочитать содержимое каждого `.md` файла.
6. Сформировать массив `AgentDefinition`, где `name` — имя файла
   без расширения `.md`.

**Расширения:**

2a. Каталог agents не существует → вернуть пустой массив
`AgentDefinition[]` (не является ошибкой).

3a. Ошибка доступа к каталогу agents (EACCES) →
`AgentDiscoverError("Failed to scan directory {dirPath}: {причина}")`.
...
3. Для каждого `AgentOutputFile` из результата `adapter.transpile`
...
3. Для каждого `AgentOutputFile` из `files` записать `content`
   в `effectiveRoot / relativePath` с кодировкой UTF-8, создавая
   промежуточные каталоги при необходимости.
4. Вернуть `AgentWriteResult`.
...
создать `AgentWriteError` с сообщением из элемента и добавить
