...
В цикле по записям (шаг 5), после подшага 5.4 (Agents):

5.5. Выполнить операцию overlay (см. § Операция overlay)
с `entry` и `projectRoot`.

Подшаг 5.5 оригинальной нумерации (отобразить результаты) сдвигается → 5.6.

**Изменения в существующих шагах:**

Шаг 5.6 (бывший 5.5: «Отобразить результаты шагов в TUI») — порядок
отображения включает Overlay: Instructions → Skills → Agents → Overlay.

Шаг 6 («Вычислить totalWritten как сумму writtenCount всех шагов») —
...
В цикле по записям (шаг 3), после подшага 3.4 (Agents):

3.5. Выполнить операцию overlay (см. § Операция overlay)
с `entry` и `projectRoot`.

Подшаг 3.5 оригинальной нумерации (отобразить результаты) сдвигается → 3.6.

**Изменения в существующих шагах:**

Шаг 3.6 (бывший 3.5: «Отобразить результаты шагов текущей записи в TUI») —
порядок отображения включает Overlay: Instructions → Skills → Agents → Overlay.

Шаг 4 («Вычислить totalWritten как суммарный writtenCount всех шагов
всех записей») — `totalWritten` вычисляется как сумма `writtenCount`
всех четырёх шагов (Instructions, Skills, Agents, Overlay).
