...
Процедура `runOverlayStep`, реализующая операцию overlay,
ЗАПРЕЩЕНО выбрасывать исключения наружу. Все ошибки, включая
фатальные (например, `LayerMergeError` в multi-layer режиме,
см. `docs/specs/layer-model.md` § Расширение «Операция overlay»,
расширение 2.7a), ДОЛЖНЫ быть возвращены как элементы массива
`errors` результирующего `TranspilerStepOutcome`. Общий контракт
шагов описан в `docs/specs/cli.md`
