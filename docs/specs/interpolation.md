...
Библиотека предоставляет три публичных метода: `loadDotenv`
для загрузки переменных окружения из `.env` файла, `buildVariables`
...
- `dotenv` (npm) — парсинг `.env` файлов.

## Загрузка .env файла

`loadDotenv(projectRoot)` — загружает переменные окружения
из `.env` файла в корне проекта и объединяет их с `process.env`.
...
3. Для каждой переменной из распарсенного файла: если переменная
   НЕ определена в `process.env` — записать значение
   в `process.env`. Если переменная уже определена
   в `process.env` — сохранить существующее значение
   (`process.env` имеет более высокий приоритет).

**Расширения:**

1a. Файл `.env` не существует → тихо пропустить (не является ошибкой),
вернуть управление без изменений `process.env`.

2a. Ошибка чтения файла (I/O-ошибка, невалидный формат) →
тихо пропустить (не является ошибкой),
вернуть управление без изменений `process.env`.
...
расширяется загрузкой `.env` файла и построением переменных
интерполяции.

**Новые шаги:**

После шага 2 (определение `projectRoot`) и перед шагом 3
(Resolve Adapters from CLI Args):

2a. Вызвать `loadDotenv(projectRoot)`
(см. § Загрузка .env файла). Загрузка выполняется один раз
и распространяется на все последующие шаги (транспилеры,
overlay step), потому что `loadDotenv` модифицирует `process.env`.
...
- Skills (шаг 4.3): `writeResults` вызывается с `options`,
  содержащим `variablesByAgentId` с картой переменных
  для текущего `entry.id`.
