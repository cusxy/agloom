...
1. Собрать значения всех определённых полей из `entry.paths`
   (skills, agents, docs, schemas). Неопределённые поля
   пропускаются.
2. Для каждого пути из шага 1 рекурсивно удалить директорию
   `<projectRoot>/<path>/` со всем её содержимым
   (файлы и поддиректории). Сама директория также удаляется.
3. Удалить каждый файл из `entry.targetFiles`
   (пути относительно `projectRoot`).
4. Сформировать `CleanOutcome` с `removedCount` (суммарное количество
   файлов, успешно удалённых на шагах 2 и 3) и `errors`.

**Расширения:**

1a. Объект `entry.paths` пустой (ни одно поле не определено) →
пропустить шаг 2, перейти к шагу 3.

2a. Директория `<projectRoot>/<path>/` не существует →
пропустить директорию, не является ошибкой.

2b. Ошибка удаления директории (EACCES и т.п.) →
добавить сообщение в `errors`, продолжить с оставшимися директориями.

3a. Файл из `targetFiles` не существует → пропустить файл,
не является ошибкой.

3b. Ошибка удаления файла из `targetFiles` (EACCES и т.п.) →
