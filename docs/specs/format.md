...
Паттерн `**/AGLOOM.md` ДОЛЖЕН исключать:
...
`agloom format [--check] [<glob>]` — форматирует или проверяет файлы проекта.
...
- `--check` (boolean, опционально, default: false) — проверить файлы
  ...
  с exit code 1.
- `<glob>` (string, опционально) — glob-паттерн для выбора файлов.
  При указании ЗАМЕНЯЕТ целевые файлы по умолчанию (не дополняет).
  При отсутствии используются целевые файлы по умолчанию
  (см. § Целевые файлы по умолчанию).

**Поведение:**

1. Распарсить аргументы `--check` и `<glob>` из командной строки.
2. Определить `projectRoot` как текущий рабочий каталог процесса
   (`process.cwd()`).
3. Определить список glob-паттернов: если `<glob>` указан — массив
   из одного элемента `[<glob>]`; если не указан — паттерны
...
- `1` — обнаружены ошибки форматирования (режим format), файлы
  ...
  ошибки выполнения инструментов, или ошибка парсинга конфига.
...
```text
Usage: agloom format [--check] [<glob>]
