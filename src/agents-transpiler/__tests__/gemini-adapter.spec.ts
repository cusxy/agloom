...
// Спецификация: docs/specs/agents-transpiler.md § Gemini адаптер

import { describe, it, expect } from "vitest";
import { GeminiAgentAdapter } from "../adapters/gemini-adapter.js";
...
describe("GeminiAgentAdapter", () => {
  describe("transpile", () => {
    ...
    // § Gemini адаптер — targetDir: ".gemini/agents"
    it('имеет targetDir равный ".gemini/agents"', () => {
      const adapter = new GeminiAgentAdapter();
      expect(adapter.targetDir).toBe(".gemini/agents");
    });

    // --- Happy path: шаги 1–2 — трансформация и формирование AgentOutputFile ---
    // § Gemini адаптер → transpile → Поведение, шаги 1–2
    it("трансформирует содержимое для gemini и возвращает definition.relativePath без ремаппинга", () => {
      const adapter = new GeminiAgentAdapter();
      ...
      // Содержимое трансформировано для gemini
      expect(files[0].content).toContain("model: gemini-2.5-pro");
