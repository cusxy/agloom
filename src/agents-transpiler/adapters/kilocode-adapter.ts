/**
 * KiloCode адаптер для agents.
 * Spec: docs/specs/agents-transpiler.md § KiloCode адаптер
 *
 * agentId: "kilocode"
...
export class KiloCodeAgentAdapter implements AgentAdapter {
  readonly agentId = "kilocode";
  readonly targetDir = ".kilo/agents";
  ...
  transpile(definitions: AgentDefinition[]): AgentOutputFile[] {
    ...
    for (const def of definitions) {
      ...
      const content = transformContent(def.rawContent, "kilocode", this.variables, this.values);
